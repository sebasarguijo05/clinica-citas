<x-app-layout>
    ...
    <div class="min-h-screen bg-slate-200 py-6">
        <div class="max-w-6xl mx-auto px-4">
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden" style="height: 75vh;">
                <div class="flex h-full">

                    {{-- Lista lateral — oculta en móvil --}}
                    <div class="hidden md:flex w-80 border-r border-slate-200 flex-col flex-shrink-0">
                    ...
                    {{-- Área del chat — ocupa todo en móvil --}}
                    <div class="flex-1 flex flex-col min-w-0">
                        ...
                        <div class="px-4 py-3 border-b border-slate-200 bg-slate-50 flex items-center gap-3">
                            ...
                            <a href="{{ route('patient.messages.index') }}"
                               class="md:hidden flex items-center justify-center w-8 h-8 rounded-lg hover:bg-slate-200 transition flex-shrink-0">
                            ...
                            <div class="w-9 h-9 bg-slate-700 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                                {{ strtoupper(substr($appointment->doctor->name, 0, 1)) }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-slate-800 truncate">{{ $appointment->doctor->name }}</p>
                            ...
                            <a href="{{ route('patient.appointments.show', $appointment) }}"
                               class="flex-shrink-0 text-xs text-slate-400 hover:text-orange-500 flex items-center gap-1 transition">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
                                </svg>
                                <span class="hidden sm:inline">Ver cita</span>
                            </a>
                        </div>

                        {{-- Mensajes --}}
                        <div class="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-slate-50" id="chat-messages">
